...
/**
 * 控制器抽象类
 * 只是用于展示覆盖在播放视频界面上的用于控制播放视频的按钮等
 */
public abstract class NineGridVideoPlayerController  extends FrameLayout {

    protected Context mContext;
    protected INineGridVideoPlayer mNineGridVideoPlayer;
    ...
    public NineGridVideoPlayerController(Context context) {
        super(context);
        mContext = context;
    }

    public void setNineGridVideoPlayer(INineGridVideoPlayer NineGridVideoPlayer) {
        mNineGridVideoPlayer = NineGridVideoPlayer;
    }
     ...
     *                  <ul>
     *                  <li>{@link NineGridVideoPlayer#STATE_IDLE}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_ERROR}</li>
     *                  <li>{@link NineGridVideoPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link NineGridVideoPlayer#MODE_NORMAL}</li>
     *                 <li>{@link NineGridVideoPlayer#MODE_FULL_SCREEN_LANDSCAPE}</li>
     *                 <li>{@link NineGridVideoPlayer#MODE_FULL_SCREEN_PORTRAIT}</li>
     *                 <li>{@link NineGridVideoPlayer#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    NineGridVideoPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        // 只有横向全屏的时候才能拖动位置、亮度、声音
        if (!mNineGridVideoPlayer.isLandscapeFullScreen()) {
            return super.onTouchEvent(event);
        }
        // 只有在播放、暂停、缓冲的时候能够拖动改变位置、亮度和声音
        if (mNineGridVideoPlayer.isIdle()
                || mNineGridVideoPlayer.isError()
                || mNineGridVideoPlayer.isPreparing()
                || mNineGridVideoPlayer.isPrepared()
                || mNineGridVideoPlayer.isCompleted()) {
            hideChangePosition();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        mGestureDownPosition = mNineGridVideoPlayer.getCurrentPosition();
                    } else if (absDeltaY >= THRESHOLD) {
                        if (mDownX < getWidth() * 0.5f) {
                            // 左侧改变亮度
                            mNeedChangeBrightness = true;
                            mGestureDownBrightness = NineGridUtil.scanForActivity(mContext)
                                    .getWindow().getAttributes().screenBrightness;
                        } else {
                            // 右侧改变声音
                            mNeedChangeVolume = true;
                            mGestureDownVolume = mNineGridVideoPlayer.getVolume();
                        }
                    }
                }
                if (mNeedChangePosition) {
                    long duration = mNineGridVideoPlayer.getDuration();
                ...
                if (mNeedChangeBrightness) {
                    ...
                    WindowManager.LayoutParams params = NineGridUtil.scanForActivity(mContext).getWindow().getAttributes();
                    params.screenBrightness = newBrightnessPercentage;
                    NineGridUtil.scanForActivity(mContext).getWindow().setAttributes(params);
                    int newBrightnessProgress = (int) (100f * newBrightnessPercentage);
                    showChangeBrightness(newBrightnessProgress);
                }
                if (mNeedChangeVolume) {
                    deltaY = -deltaY;
                    int maxVolume = mNineGridVideoPlayer.getMaxVolume();
                    int deltaVolume = (int) (maxVolume * deltaY * 3 / getHeight());
                    int newVolume = mGestureDownVolume + deltaVolume;
                    newVolume = Math.max(0, Math.min(maxVolume, newVolume));
                    mNineGridVideoPlayer.setVolume(newVolume);
            ...
            case MotionEvent.ACTION_UP:
                if (mNeedChangePosition) {
                    mNineGridVideoPlayer.seekTo(mNewPosition);
                    hideChangePosition();
